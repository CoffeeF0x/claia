...
def zammad_run_process(settings: Settings, zammad: ZammadAPI) -> Result:
  tickets = zammad.list_tickets()

  temp_ticket = tickets[0]  # Select the first ticket for demonstration purposes
  ticket_details = zammad.get_ticket_details(temp_ticket)
  summary_messagess = [
  ...
  # summarize ticket
  summary_result = model_run(settings.active_model, summary_messagess, settings=settings)
    ...
    if tagging_result.is_error():
      ...
      if tagging_result.data and "[TAG]" in tagging_result.data:
        ...
        if tag in ["Phishing", "Spam", "Completed", "NetworkHardware", "Jenzabar", "LMS", "Report", "Printers", "Forms", "Adobe", "InfoMaker", "Salesforce", "Classroom", "Login", "Student", "Filter", "Video", "NoCategoryFound"]:
          zammad.add_tag(temp_ticket, f"AI-{tag}")
        else:
          zammad.add_tag(temp_ticket, "AI-Unknown")
