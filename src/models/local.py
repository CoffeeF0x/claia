...
class MiniCPM3_4B_LocalModel(TransformersLocalModel):
    def __init__(self, model_name: str, model_path: str, defer_loading: bool = False, device="cpu", log_level: str = "info"):
        model_path = os.path.join(model_path, "MiniCPM3-4B")
        super().__init__(model_name, model_path, defer_loading, device, log_level)
...
class Qwen2p5_32B_InstructLocalModel(TransformersLocalModel):
    def __init__(self, model_name: str, model_path: str, defer_loading: bool = False, device="cpu", log_level: str = "info"):
        model_path = os.path.join(model_path, "Qwen2.5-32B-Instruct")
        super().__init__(model_name, model_path, defer_loading, device, log_level)

    def generate(self, messages: list, **kwargs) -> str:
        kwargs['max_new_tokens'] = kwargs.get('max_new_tokens', 8192)
        return super().generate(messages, **kwargs)

    def download(self, model_path: str) -> None:
        logging.info(f"Downloading Qwen2.5-32B-Instruct model to {model_path}")
        os.makedirs(model_path, exist_ok=True)
