...
- `base.py` — the post-overhaul `BaseProtocol` ABC. Every protocol
  exposes:
...
- `_legacy.py` — pre-overhaul `BaseProtocol` ABC retained behind a
  `DeprecationWarning` for downstream extensions that haven't
  migrated yet.

## How protocols fit in (post-overhaul)

1. The framework loads each `claia.tool_protocols` plugin via
   pluggy. The simple protocol additionally has its native tool
...
4. Inside the agent loop the streaming `TagParser` emits a
   ...
   emitted as a `"token"` event so terminal renderers see the call
