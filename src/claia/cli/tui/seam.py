"""
The kintsugi seam: a one-line crack carrying the bound track's
liveness.

The crack is a deterministic pattern of line characters (seeded per
width) with occasional thick veins. At rest it sits in the resting
border color with the veins in dimmed gold. While the bound track
works, glints of gold travel the crack — warm amber while a tool
call is in flight — and a failed or cancelled turn flashes the whole
line in the matching semantic color before decaying back to rest.

Pure display: the switchboard pushes :class:`Phase` changes and
flashes; the widget owns its own timer and pauses it whenever there
is nothing left to animate. When Textual's animation level is
``none`` the seam renders static state colors instead of moving.
"""
...
class Phase(Enum):
  """What the bound track is doing, as far as display cares."""
  IDLE = "idle"
  STREAMING = "streaming"
  TOOL = "tool"
...
#                              FUNCTIONS                               #
########################################################################
def crack_pattern(width: int) -> str:
  """A deterministic crack for ``width`` cells.

  Seeded by width so resizes are stable: runs of plain line broken
  ...
  while len(cells) < width:
    roll = rng.random()
    if roll < 0.10:
      cells.extend("━" * rng.randint(2, 4))
    elif roll < 0.22:
      cells.extend("╌" * rng.randint(1, 2))
    else:
      cells.extend("─" * rng.randint(4, 9))
...
########################################################################
class Seam(Widget):
  """One line of crack between transcript and composer."""
  ...
  def _tick(self) -> None:
    ...
    self._offset = (self._offset + SPEED) % width
    if self._flash > 0:
  ...
  def render(self) -> Text:
    ...
    if width <= 0:
      return Text("")
    if len(self._pattern) != width:
      self._pattern = crack_pattern(width)
    ...
    if active:
      ...
      vein = core
    centers = [
      (self._offset + k * width / GLINTS) % width for k in range(GLINTS)
    ]
    ...
    for i, char in enumerate(self._pattern):
      ...
      if active and animated:
        glow = self._glow(i, centers, width)
        if glow >= 0.95:
          color = tip
        elif glow > 0:
  ...
  @staticmethod
  def _glow(cell: int, centers, width: int) -> float:
